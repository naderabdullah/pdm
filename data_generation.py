    ...
    for file_name in os.listdir(input_dir):
        ...
        if file_name.endswith('.json'):
            ...
            if 'sensorValues' in data and 'Probe_Temp' in data['sensorValues'] and data['sensorValues']['Probe_Temp']:
                ...
                if probe_temp_value < 4: #or probe_temp_value > 13:
            ...
            for key in data['sensorValues']:
                for entry in data['sensorValues'][key]:
                    entry['timestamp'] = datetime.strptime(entry['timestamp'], '%Y-%m-%dT%H:%M:%S.%f').strftime('%m/%d/%Y %H:%M:%S.%f')
...
def generate_bad_records(original_dicts, output_dir):
    ...
    for i in range(400):
        ...
        formatted_timestamp = timestamp.strftime('%m/%d/%Y %H:%M:%S.%f')
        ...
        for key in original_dicts[0].keys():
            if key == 'File':
                ...
                max_val = 30
                new_record['sensorValues'][key] = [{"timestamp": formatted_timestamp, "value": str(round(random.uniform(min_val, max_val), 2))}]
            else:
